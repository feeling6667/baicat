...
3. **每个分镜上方预留干净留白区**：**生图时不画任何文字**，只留出顶部空白，旁白由作者后期手动添加，构图不拥挤。
...
- 彩铅生图始终不画文字，仅保留此留白区供后期加字。

**信息分工**：
- 画面 + 后期加的旁白：承载故事**主线骨架**与视觉情绪。
- 作品正文（HTML/推文正文）：补充**人物背景、对话、细节铺垫、完整故事逻辑**。
- 即：图上方留白区只放主线短句（后期加字），背景与前因后果放正文文字，二者互补不重复。
...
- 模型：`gpt-image-2`
- 尺寸：`1024x1536`（竖屏条漫比例，竖版单图/封面用）
- 质量：`high`
- 返回：`b64_json`，需 base64 解码保存为 JPEG

> 注：split2 双分镜模式下，每个分镜单元单独以 `1024x1024` 方形高清生成，再由脚本用
> Pillow 拼接为 `1024x1536` 双分镜图（中间 #d9d9d9 淡灰细线），保证分镜细节完整、笔触不压缩。
